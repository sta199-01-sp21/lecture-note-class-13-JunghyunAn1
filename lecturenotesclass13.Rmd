...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-JunghyunAn1/manhattan.csv")
```
...
We are 95% confident that the proportion of mice that died while not on 
treatment is between 0.690 and 0.793
...
```{r practice_2_ci}
ci_mice_live <- get_ci(boot_prop, level = .90)
ci_mice_live
```
...
We are 90% confident that the proportion of mice that lived while on treatment 
is between 0.690 and 0.793
